...
def scale_a(data_numpy, scale=0.2, p=0.5):
    if random.random() < p:
        # Apply scaling across 9 channels
        scale_factors = 1 + np.random.uniform(-1, 1, size=(9, 1, 1, 1)) * np.array(scale)
        return data_numpy * scale_factors
    else:
        return data_numpy.copy()

''' AimCLR '''
transform_order = {'ntu': [0, 1, 2, 3, 8, 9, 10, 11, 4, 5, 6, 7, 16, 12, 13, 14, 15]}
...
def shear_a(data_numpy, s1=None, s2=None, p=0.5):
    if random.random() < p:
        temp = data_numpy.copy()

        # Extend s1 and s2 to match the 9-channel configuration
        if s1 is not None:
            s1_list = s1
        else:
            s1_list = [random.uniform(-0.5, 0.5) for _ in range(9)]

        if s2 is not None:
            s2_list = s2
        else:
            s2_list = [random.uniform(-0.5, 0.5) for _ in range(9)]

        # Create the 9x9 shear matrix
        R = np.eye(9)  # Start with an identity matrix
        for i in range(9):
            for j in range(i+1, 9):
                R[i, j] = s1_list[j-i]  # Shearing factors for s1
                R[j, i] = s2_list[j-i]  # Shearing factors for s2
...
class joint2bone(nn.Module):
    def __init__(self):
        ...
        self.pairs = [(1, 2), (1, 4), (2, 3), (4, 5), (5, 6), (1, 7), (7, 8), (7,10),
                    (8, 9), (4, 10), (10, 11), (11, 12), (0, 13), (0, 14),
                    (13, 15), (14, 16)]
...
class bone2joint(nn.Module):
    def __init__(self):
        ...
        self.center = 0
        self.pairs_1 = [(1, 2), (1, 4), (2, 3)]
        self.pairs_2 = [(4, 5), (5, 6), (1, 7)]
        self.pairs_3 = [(7, 8), (7, 10)]
        self.pairs_4 = [(8, 9), (4, 10), (10, 11)]
        self.pairs_5 = [(11, 12), (0, 13), (0, 14)]
        self.pairs_6 = [(13, 15), (14, 16)]
